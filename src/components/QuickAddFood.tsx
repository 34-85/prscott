...
/** Nice label for the food's native serving. */
...
/**
 * Structured food entry: search a food, pick quantity + unit, live macro
 * preview, log as a single meal. Numbers are computed by the same
 * estimateMeal path used by chat + structured MealLogger.
 */
export function QuickAddFood({ date, onClose }: Props) {
  const { state, addMeal } = useStore()
  const db = useMemo(() => getFoodDatabase(state.customFoods), [state.customFoods])
  ...
  const matches = useMemo(() => {
    ...
    if (!q) {
      // Empty query → show a curated top strip of common PSMF anchors
  ...
  // Preview macros by routing through the normal estimator.
  const preview = useMemo(() => {
    if (!picked) return null
    const n = parseFloat(qty)
    if (!n || n <= 0) return null
    const rawText = buildRawText(n, unit, picked)
    return estimateMeal(rawText, state.customFoods)
  }, [picked, qty, unit, state.customFoods])

  function commit() {
    if (!picked || !preview) return
    const n = parseFloat(qty)
    if (!n || n <= 0) return
    addMeal(date, buildRawText(n, unit, picked))
    onClose()
  }
      ...
      <div className="relative w-full sm:max-w-lg">
        <div className="rounded-t-3xl border border-ink-line bg-ink-card p-5 pb-8 sm:rounded-3xl">
          ...
          <input
            ...
            className="field mt-4 text-base"
          />
            ...
            <div className="mt-3 max-h-56 overflow-y-auto rounded-xl border border-ink-line bg-ink-soft/40">
              {matches.length === 0 ? (
                <p className="px-3 py-6 text-center text-sm text-mute-soft">
                  No matches. Try a shorter word — or use chat mode to describe it in words.
                </p>
              ) : (
                matches.map((f) => (
                  <button
...
function buildRawText(qty: number, unit: string, food: FoodEntry): string {
  // Use the first alias so the matcher lands on the picked food exactly. If
  // the food's unit matches a natural noun (banana / apple / berry), skip the
  // unit token — "2 banana" reads worse than "2 bananas" and the parser
  // handles bare integers as counts. We prefer explicit for reproducibility.
