# Pets of [City] — Strategic Rationale ("The Why")

> The purpose behind the business. PoC is **not (only) a revenue play** — it is the **local community + connectivity layer that amplifies, measures, and helps fund APPA's mission.** This should anchor the MVP, the eventual business plan, and the board case.
...
| APPA initiative (today, an expense) | Why it's local | How PoC amplifies it |
|---|---|---|
| **Independent pet retailer support** | Independents live or die on local foot traffic; threatened by consolidation **and** local sale bans | Directory + featured listings + discount card drive **local foot traffic**; generates **local economic-impact data** to defend the channel |
| **Pet-inclusive housing** (to grow pet ownership) | Housing policy & supply are hyper-local | Pet-friendly housing listings + **local demand data** + advocacy amplification at the community level |
...
1. **Reach to the local level** APPA has never had — where bans, housing, and adoption actually happen.
2. **Evidence.** A first-party local data panel produces the **economic-impact and demand data** that make APPA's advocacy (independent retail, housing, anti-ban) far more persuasive — quantified, local, and current.
3. **Mobilization.** A community of engaged local pet owners is an advocacy asset (turnout, comment, support) at the community level where it counts.
4. **A funding flywheel.** The same activities that advance the mission also generate revenue (insurance/housing/lead-gen, card, events, data) — so the ~$2.5M of mission work becomes **partly self-funding** instead of pure expense.
